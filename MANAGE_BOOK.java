...
public class MANAGE_BOOK extends javax.swing.JFrame {
    ...
    public MANAGE_BOOK() {
        initComponents();
        Model = (DefaultTableModel) BOOK.getModel(); // Get Table Model
   ...
   private void updateDatabase(int row) {
        String updateQuery = "UPDATE BOOK SET TITLE=?, BOOK_NO=?, AUTHOR=?, CATEGORY=?, ISBN=?, PUBLISHER=?, PUBLISH_YEAR=?, PRICE=?, QUANTITY=?, BOOK_ID =? WHERE ID=?";

        try (Connection conn = DriverManager.getConnection(url, user, pass);
             PreparedStatement pstmt = conn.prepareStatement(updateQuery)) {

            int id = (int) Model.getValueAt(row, 0); // ID column (Primary Key)
            String book_id = (String) Model.getValueAt(row, 1);
            String book_no = (String) Model.getValueAt(row, 2);
            String title = (String) Model.getValueAt(row, 3);
            String isbn = (String) Model.getValueAt(row, 4);
            String author = (String) Model.getValueAt(row, 5);
            String category = (String) Model.getValueAt(row, 6);
            String publisher = (String) Model.getValueAt(row, 7);
            String publish_year = (String) Model.getValueAt(row, 8);

            // Parse String to int for price and quantity
            int price = Integer.parseInt((String) Model.getValueAt(row, 9).toString()); // Parse price
            int quantity = Integer.parseInt((String) Model.getValueAt(row, 10).toString()); // Parse quantity
            ...
            pstmt.setString(7, publish_year);
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error updating book data: " + e.getMessage());
        } catch (NumberFormatException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error parsing numeric values (price or quantity): " + e.getMessage());
        }
    }
    ...
    private void initComponents() {
        ...
        BOOK.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "BOOK_ID", "BOOK NO", "TITLE", "ISBN", "AUTHOR", "CATEGORY", "PUBLISHER", "PUBLISH YEAR", "PRICE", "QUANTITY"
            }
        ) {
        ...
        FILTERS.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "ID", "BOOK_NO", "TITLE", "CATEGORY", "AUTHOR", "ISBN", "PUBLISHER", "PUBLISH_YEAR", "PRICE", "QUANTITY", " ", " ", " ", " ", " ", " " }));
    ...
    int selectedRow = BOOK.getSelectedRow(); // Get selected row
